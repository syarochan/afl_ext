...
#define ALLOC_C1(_ptr)  (((size_t*)(_ptr))[-2])
#define ALLOC_S(_ptr)   (((size_t*)(_ptr))[-1])
#define canary_num(_num) ({ \
      size_t _tmp = (size_t)_num; \
    if ((size_t)_tmp % SIZE != 0) _tmp += SIZE; \
      _tmp;\
      })

#define ALLOC_C2(_ptr)  (((size_t*)(_ptr))[canary_num(ALLOC_S(_ptr)) / SIZE]) 

#define ALLOC_OFF_HEAD SIZE * 2
#define ALLOC_OFF_TOTAL (ALLOC_OFF_HEAD + 1)
#define HEAP_CANARY_SIZE SIZE                         // heap_canary size
...
struct list_canary{
	size_t index;                      // heap_canary index 0 ~ 2046
   size_t list_idx;                    // second list index
	size_t next;                       // next list
	size_t flag;                       // init:0, not yet:1
	size_t * list[256];                // heap_canary_ptr
   struct free_list * free_list;   // free heap canary list
};

struct list_canary list_s = {0, 0, 0, 0, {0}, 0};

static inline size_t store_heap_canary(size_t heap_canary, void* ptr , u32 size){
	size_t * victim = 0;
   size_t * victim_list = 0;
   struct free_list * f_list = list_s.free_list;
   size_t header = 0;

	if(!list_s.flag){
		if(!(list_s.list[0] = (size_t*)malloc(256 * SIZE)))
         ABORT("BAD ALLOC MEMORY");
		memset(list_s.list[0], 0x0, 256 * SIZE);
      victim_list = list_s.list[0];
      if(!(victim_list[0] = (size_t)malloc(2047 * SIZE)))
         ABORT("BAD ALLOC MEMORY");
      memset((size_t*)victim_list[0], 0x0, 2047 * SIZE);
      list_s.flag = 1;
   }
      ...
      if(list_s.list_idx < 256){
         ...
         if(!(victim_list[list_s.list_idx] = (size_t)malloc(2047 * SIZE)))
            ABORT("BAD ALLOC MEMORY");
         memset((size_t*)victim_list[list_s.list_idx], 0x0, 2047 * SIZE);
      }
      if(list_s.list_idx == 256 && list_s.next < 255){
         list_s.list_idx = 0;
         list_s.next++;
         if(!(list_s.list[list_s.next] = (size_t*)malloc(256 * SIZE)))
            ABORT("BAD ALLOC MEMORY");
         memset(list_s.list[list_s.next], 0x0, 256 * SIZE);
         victim_list = (size_t*)list_s.list[list_s.next];
         if(!(victim_list[list_s.list_idx] = (size_t)malloc(2047 * SIZE)))
            ABORT("BAD ALLOC MEMORY");
         memset((size_t*)victim_list[list_s.list_idx], 0x0, 2047 * SIZE);
      }
   ...
   if(list_s.free_list > 0){
      ...
      victim = (size_t*)victim_list[f_list->list_idx_2];
   ...
   while(list_s.index < 2047){
      victim_list = list_s.list[list_s.next];
      victim      = (size_t*)victim_list[list_s.list_idx];
      if(victim[list_s.index] == (size_t)NULL){
...
static inline size_t form_heap_canary(){
   int fd;
   size_t buf[SIZE] = {0};
...
static inline size_t check_heap_canary(void* ptr){
   size_t heap_canary           = ALLOC_C2(ptr);
   size_t victim_index          = IDX_PTR(ptr);
   size_t victim_1st_list_index = LIST_PTR(ptr);
   size_t victim_2nd_list_index = LIST_IDX_PTR(ptr);
   size_t * victim_list           = list_s.list[victim_1st_list_index];
   size_t * victim                = (size_t*)victim_list[victim_2nd_list_index];
...
static inline size_t free_heap_canary(void* ptr){
   size_t victim_index            = IDX_PTR(ptr);
   size_t victim_1st_list_index   = LIST_PTR(ptr);
   size_t victim_2nd_list_index   = LIST_IDX_PTR(ptr);
   size_t * victim_list           = list_s.list[victim_1st_list_index];
   ...
   victim[victim_index] = (size_t)NULL; // heap canary init
...
static inline void* DFL_ck_alloc_nozero(u32 size){
	void* ret;
   size_t heap_canary = form_heap_canary();
